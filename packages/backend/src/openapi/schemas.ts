...
export const PaginationMetaSchema = z.object({
  ...
  nextCursor: z
    .string()
    .optional()
    .openapi({ description: 'Cursor for next page (if using cursor-based pagination)' }),
});
...
export const UserSchema = z.object({
  ...
  photoURL: z
    ...
    .url()
    .nullable()
    .openapi({ description: 'User profile photo URL', example: 'https://example.com/photo.jpg' }),
  groupIds: z
    .array(z.string())
    .openapi({ description: 'IDs of the groups the user belongs to', example: ['group_admin'] }),
  isSuperAdmin: z
    .boolean()
    .openapi({ description: 'Whether the user is a super administrator', example: false }),
  status: z
    .enum(['active', 'disabled'])
    .openapi({ description: 'User account status', example: 'active' }),
  disabledAt: z
    .string()
    .datetime()
    .optional()
    .openapi({ description: 'When the user was disabled' }),
  disabledBy: z.string().optional().openapi({ description: 'ID of user who disabled this user' }),
  createdAt: z
    .string()
    .datetime()
    .openapi({ description: 'When the user was created', example: '2024-01-15T10:30:00Z' }),
  updatedAt: z
    .string()
    .datetime()
    .openapi({ description: 'When the user was last updated', example: '2024-01-15T10:30:00Z' }),
  lastLoginAt: z
    .string()
    .datetime()
    .openapi({ description: 'When the user last logged in', example: '2024-01-15T10:30:00Z' }),
...
export const UserWithPermissionsSchema = UserSchema.extend({
  permissions: z
    .array(z.string())
    .openapi({ description: 'List of permission strings', example: ['users:read', 'users:list'] }),
  groupNames: z
    .array(z.string())
    .openapi({ description: 'Names of the user groups', example: ['Administrators'] }),
});
...
export const CreateUserSchema = z.object({
  email: z
    .string()
    .email()
    .openapi({ description: 'User email address', example: 'newuser@example.com' }),
  ...
  photoURL: z
    ...
    .nullable()
    .optional()
    .openapi({ description: 'User profile photo URL' }),
  groupIds: z
    .array(z.string())
    .optional()
    .openapi({ description: 'IDs of the groups to assign the user to' }),
});
...
export const UpdateUserSchema = z.object({
  displayName: z
    ...
    .max(100)
    .optional()
    .openapi({ description: 'User display name', example: 'John Smith' }),
  photoURL: z
    .string()
    .url()
    .nullable()
...
export const ChangeGroupSchema = z.object({
  groupId: z
    .string()
    .min(1)
    .openapi({ description: 'ID of the new group', example: 'group_users' }),
});
...
export const UserSearchQuerySchema = PaginationQuerySchema.extend({
  status: z
    ...
    .optional()
    .openapi({ description: 'Filter by user status', example: 'active' }),
...
export const CreateGroupSchema = z.object({
  ...
  description: z
    ...
    .max(500)
    .default('')
    .openapi({ description: 'Group description', example: 'Users who can edit content' }),
  permissions: z
    .array(z.string())
    .optional()
    .openapi({
      description: 'List of permissions to assign',
      example: ['users:read', 'users:list'],
    }),
});
...
export const GroupQuerySchema = z.object({
  includeUserCounts: z
    .enum(['true', 'false'])
    .optional()
    .openapi({ description: 'Include user count for each group' }),
});
...
export const AuditLogSchema = z.object({
  ...
  timestamp: z
    .string()
    .datetime()
    .openapi({ description: 'When the action occurred', example: '2024-01-15T10:30:00Z' }),
  actorId: z
    .string()
  ...
  action: AuditActionSchema.openapi({
    description: 'Type of action performed',
    example: 'USER_UPDATED',
  }),
  resource: AuditResourceSchema.openapi({
  ...
  ipAddress: z
    .string()
    .optional()
    .openapi({ description: 'IP address of the actor', example: '192.168.1.1' }),
...
export const AuditSearchQuerySchema = PaginationQuerySchema.extend({
  ...
  startDate: z
    .string()
    .optional()
    .openapi({ description: 'Filter by start date (ISO 8601)', example: '2024-01-01T00:00:00Z' }),
  endDate: z
    .string()
    .optional()
    .openapi({ description: 'Filter by end date (ISO 8601)', example: '2024-12-31T23:59:59Z' }),
  sortOrder: z
    .enum(['asc', 'desc'])
    .optional()
    .openapi({ description: 'Sort order', example: 'desc' }),
...
export const AuditUserQuerySchema = z.object({
  limit: z
    .string()
    .optional()
    .openapi({ description: 'Maximum number of logs to return', example: '50' }),
});
...
export const AppFeaturesSchema = z.object({
  auditLogging: z
    .boolean()
    .openapi({ description: 'Whether audit logging is enabled', example: true }),
  userRegistration: z
    .boolean()
    .openapi({ description: 'Whether user registration is enabled', example: true }),
});
...
export const SettingsSchema = z.object({
  ...
  defaultGroupId: z
    .string()
    .openapi({ description: 'Default group ID for new users', example: 'group_users' }),
...
export const UpdateSettingsSchema = z.object({
  appName: z
    ...
    .max(100)
    .optional()
    .openapi({ description: 'Application name', example: 'My Dashboard' }),
  defaultGroupId: z
    .string()
    .min(1)
    .optional()
    .openapi({ description: 'Default group ID for new users' }),
  features: z
    .object({
...
export const LoginRequestSchema = z.object({
  idToken: z
    .string()
    .min(1)
    .openapi({ description: 'Firebase ID token from client authentication' }),
});
...
export const PermissionDefinitionSchema = z.object({
  ...
  description: z
    .string()
    .openapi({ description: 'Permission description', example: 'View user details' }),
});
...
export const PermissionListResponseSchema = z.object({
  ...
  byResource: z
    ...
    .openapi({ description: 'Permissions grouped by resource' }),
...
export const ErrorResponseSchema = z.object({
  ...
  error: z.object({
    ...
    message: z
      .string()
      .openapi({ description: 'Error message', example: 'Authentication required' }),
